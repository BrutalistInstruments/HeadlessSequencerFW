...
static midiEvent_t nullMidiEvent = { 0,0,0,0,0,0,0 };
...
void writeEvent(midiEvent_t inMidiEvent)
{
    ...
    //while(sdRam[sdRamIndex] != nullMidiEvent)
    while(!isNullMidiEvent(sdRam[sdRamIndex]))
    {
        sdRamIndex++;
    }
...
void deleteEvent(midiEvent_t inMidiEvent)
{
    ...
    uint32_t deleteAddress = (uint32_t)eventToDelete;
    uint32_t ramDeleteIndex = (deleteAddress-SDRAM_ADDRESS_START) >> 4;
    sdRam[ramDeleteIndex] = nullMidiEvent;
...
{
    if(
        inMidiEvent.midiByte1 == nullMidiEvent.midiByte1 &&
        inMidiEvent.midiByte2 == nullMidiEvent.midiByte2 &&
        inMidiEvent.midiByte3 == nullMidiEvent.midiByte3 &&
        inMidiEvent.msTimeStamp == nullMidiEvent.msTimeStamp &&
        inMidiEvent.nextEvent == nullMidiEvent.nextEvent &&
        inMidiEvent.prevEvent == nullMidiEvent.prevEvent &&
        inMidiEvent.status == nullMidiEvent.status
    )
    {
        return true;
